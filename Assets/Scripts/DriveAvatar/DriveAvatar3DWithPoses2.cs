...
public class DriveAvatar3DWithPoses2 : MonoBehaviour
{
    // define the retargeting contents
    // public bool ShowHumanSkeleton = true;
    ...
    // define the bones, fixed bones, joints, bone lengths, T-Pose rotations, pose rotations of the avatar and T-Pose rotations of the human
    ...
    // define the pose of the human(for joint rotation/position control)
    // public Poses5 humanPose = new Poses5(new List<Vector3>(), new List<float>());
    public Poses5 humanPose = new Poses5(new List<Vector3>(), new List<float>()){
    ...
    // public Poses5 humanTPose = new Poses5(new List<Vector3>(), new List<float>());
    public Poses5 humanTPose = new Poses5(new List<Vector3>(), new List<float>()){
        Positions=new List<Vector3>(){
            new Vector3(0f, 0f, 0f),
            new Vector3(0f, 0f, 0f),
    ...
    List<Quaternion> GetHumanTPoseRotations(List<Vector3> vectorHumanTPose)
    {
        ...
        // construct the Poses5 class for calculating the human T-Pose rotations
        humanTPose = new Poses5(vectorHumanTPose, avatarBoneLengths);
    ...
    void RetargetTransform()
    {
        ...
        humanPose = new Poses5(vectorHumanKPs, avatarBoneLengths);

        // calculate the pose of the avatar(reference:world)
        for(int i = 0; i < avatarBones.Count; i++)
        {
            avatarPose[i].rotation = humanPose.Rotations[i] * Quaternion.Inverse(humanTPoseRotations[i]) * avatarTPoseRotations[i];
        }
    ...
    void SetTransform()
    {
        ...
        int[] fixedBones = new int[] {};

        // update the pelvis(hip) position of the avatar if the option picked
        if(RetargetRootLocation)
        {
            avatarBones[6].transform.position = humanPose.FKPositions[6]; // 6 stand for pelvis
            // avatarBones[6].transform.position = humanPose.Positions[6]; // 6 stand for pelvis    
        }
        ...
        if(RetargetPose)
        {
            for(int i = 0; i < avatarBones.Count; i++)
            {
                ...
                if(!fixedBones.Contains(i))
                {
                    ...
                    avatarBones[i].transform.rotation = avatarPose[i].rotation; // for world rotation
                    // Debug.Log("avatarPose[i].localRotation: " + avatarPose[i].localRotation); // for debug
                    // avatarBones[i].transform.localRotation = avatarPose[i].LocalRotations; // for local rotation
                }
            }
            Debug.Log("avatarPose[6].rotation: " + avatarPose[6].rotation); // for debug
        }
    }
