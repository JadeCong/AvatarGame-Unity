...
public class DriveAvatar3DWithPoses5 : MonoBehaviour
{
    ...
    public bool UseAbsoluteCoordinate = true;
    public bool UseLocalRotation = false;
    public bool UseForwardKinematics = true;
    public bool SetFootOnGround = true;
    ...
    // define the data format of human joint keypoints as int frame and Vector3 list keypoints from the broker
    public int HumanPoseFrame = 0; // for picked frame of human poses for driving avatar
    ...
    public Poses7 humanTPose;
    public Poses7 humanPose;
    // public Poses7 humanTPose = new Poses7(new List<Vector3>(), new List<float>()){
    //     Frame = 0,
    ...
    //     UseAbsoluteCoordinate = true,
    //     UseLocalRotation = false,
    //     UseForwardKinematics = true,
    //     SetFootOnGround = true,
    ...
    //         new Vector3(0f, 0f, 0f),},
    // };
    // public Poses7 humanPose = new Poses7(new List<Vector3>(), new List<float>()){
    //     Frame = 0,
    //     Positions=new List<Vector3>(){
    //         new Vector3(0f, 0f, 0f),
    //         new Vector3(0f, 0f, 0f),
    ...
    List<Quaternion> GetHumanTPoseRotations(List<Vector3> vectorHumanTPose)
    {
        ...
        // construct the Poses7 class for calculating the human T-Pose rotations
        humanTPose = new Poses7(HumanPoseFrame, vectorHumanTPose, avatarBoneLengths, UseAbsoluteCoordinate, UseLocalRotation, UseForwardKinematics, SetFootOnGround);
    ...
    void RetargetTransform()
    {
        ...
        (HumanPoseFrame, vectorHumanKPs) = EncodeHumanKPs(playerMsgs);
        ...
        if(UseFilter)
        {
            ...
            if(SetLowpassFilter)
            {
                for(int j = 0; j < vectorHumanKPs.Count; j++)
                {
                    if(LowpassFilterOrder == 1)
                    {
                        vectorHumanKPs[j] = lowpassFilters[j].FirstOrderLowpassFilter(vectorHumanKPs[j], LowpassParamAlpha); // first order
                    }
                    else if(LowpassFilterOrder == 2)
                    {
                        vectorHumanKPs[j] = lowpassFilters[j].SecondOrderLowpassFilter(vectorHumanKPs[j], LowpassParamAlpha); // second order
                    }
                    else if(LowpassFilterOrder == 3)
                    {
                        vectorHumanKPs[j] = lowpassFilters[j].ThirdOrderLowpassFilter(vectorHumanKPs[j], LowpassParamAlpha); // third order
                    }
                    else
                    {
                        throw new ArgumentException("Wrong lowpass filter order and please set it to int 1, 2 or 3 order.", nameof(LowpassFilterOrder));
        ...
        humanPose = new Poses7(HumanPoseFrame, vectorHumanKPs, avatarBoneLengths, UseAbsoluteCoordinate, UseLocalRotation, UseForwardKinematics, SetFootOnGround);
        ...
        if(!UseAbsoluteCoordinate)
        {
            ...
            for(int i = 0; i < avatarBones.Count; i++) // 15 rotations for avatarPose in avatarBones order
            {
        ...
        if(UseFinalIK || UseForwardKinematics || ShowHumanSkeleton)
        {
            ...
            for(int i = 0; i < avatarJoints.Count; i++) // 16 positions for avatarPose in avatarJoints order
            {
        ...
        {
            RefreshHumanMarkers(humanPose.FKPositions, OffsetsFromSkeletonToAvatar);
        }
    }
    ...
    void SetTransform()
    {
        ...
        int[] setJoints = new int[] {0, 5, 6, 10, 15}; // for final IK control
        ...
        if(UseFinalIK)
        {
            ...
            for(int i = 0; i < avatarJoints.Count; i++)
            {
                ...
                if(setJoints.Contains(i))
                {
                    ...
                    //TODO: change the right bone roataions
                    avatarJoints[i].transform.rotation = avatarPose[i].rotation; // world reference
                }
            }
        }
        else
